...
class TestCheckingAccount(unittest.TestCase):
    ...
    def setUp(self):
        self.checking1 = CheckingAccount(TestCheckingAccount.ACCOUNT_NUMBER, TestCheckingAccount.CHECKING_ONE_BALANCE)
        self.checking2 = CheckingAccount(TestCheckingAccount.ACCOUNT_NUMBER, TestCheckingAccount.CHECKING_TWO_BALANCE)
        self.checking3 = CheckingAccount(TestCheckingAccount.ACCOUNT_NUMBER)
        self.checking4 = CheckingAccount(TestCheckingAccount.ACCOUNT_NUMBER, TestCheckingAccount.CHECKING_FOUR_BALANCE)
        
        # test to ensure assertions function in constructor
        for i in range(len(TestCheckingAccount.INVALID_INPUTS)):
            with self.assertRaises(AssertionError):
                self.checking5 = CheckingAccount(TestCheckingAccount.ACCOUNT_NUMBER, TestCheckingAccount.INVALID_INPUTS[i])
    ...
    def testAddTransaction(self):
        account = CheckingAccount(TestCheckingAccount.ACCOUNT_NUMBER)
