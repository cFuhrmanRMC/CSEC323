...
class Client:
    ...
    def __init__(self,firstName: str, lastName: str, phone: str, address: list, accountType: str, password: str):
        ...
        self._password = Password(password, self._salt)

        # set the inital account number
    ...
    # @return: an integer, the c.checkpwlient number
    ...
    def changePassword(self, password: str)->None:
        ...
        userInput = Password(password, self._salt)
        ...
        while (userInput != self._password):
            ...
            try:
                userInput.changePassword(temp, self._salt)
            except:
        ...
        # Set new password
        self._salt = bcrypt.genSalt()
        self._password = Password(newPass, self._salt)
    ...
    def openAccount(self, accountType: str):
        ...
        if accountType == "Checking":
            self._accounts.append(CheckingAccount(self._nextAccountNumber, 0.0))
            
        else:
            self._accounts.append(SavingsAccount(self._nextAccountNumber, 0.0))
            
        
  
        self._nextAccountNumber = self._nextAccountNumber + 1
...
if __name__ == '__main__':
    ...
    newClient = Client("Johnny", "Cash", "8045559042", address, "Checking", "dey832120")
   ...
   # print("success")
    ...
    client2 = Client("somthing", "else", "1231231234", address, "Checking", "sdf234fr3")
    
    client2.openAccount("Savings")
    client2.openAccount("Savings")
    
    secondList = client2._getAccounts()
    ...
    print(client2.closeAccount(secondList[1])  )  
    
    print(client2)
    
    secondList[0].deposit(500.00)
    secondList[0].withdrawal(10.00)
    secondList[0].addInterest()
    print(secondList[0])
