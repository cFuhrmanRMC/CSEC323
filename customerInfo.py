...
class Password():
    ...
    def __init__(self, password: str, salt: bytes):
        ...
        # ensure salt is correct in length/type
        assert isinstance(salt, bytes), "Invalid type: salt must be bytes"
        
        
        #assert sys.getsizeof(salt) == 16, "Invalid size: salt must be 16 bytes"
        
        # iterate password and compare characters
        badChar = False
        for element in password:

                # Early stopping if bad char_saltacter exists
                assert element not in Password.INVALID_CHARS, "Password cannot contain invalid characters"
                if (not badChar):
                    badChar = element in Password.INVALID_CHARS
                else:
                    raise Exception("Password cannot contain invalid characters")

        # Generate salt, hash password with salt
        self._salt = salt
        self._HashedPass = self.saltAndHash(password, salt)
    ...
    #@ensure password is hashed and not as plain text
    def saltAndHash(self, password: str, salt: bytes) ->bytes:
        ...
        # ensure salt is correct in length/type
        assert isinstance(salt, bytes), "Invalid type: salt must be bytes"
        temp = salt.decode('utf-8')
        #assert sys.getsizeof(temp) == 16, "Invalid size: salt must be 16 bytes"        
        ...
        hashedPass = bcrypt.hashpw(passwordBytes, salt)
