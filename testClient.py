...
class TestClient(unittest.TestCase):
    ...
    def setUp(self):
        ...
        invalidFirstNames = [
            "Dilllllllllllllllllllllllllllllllon", "", "D1ll@n", 5]

        # test to ensure name works correctly (len 1-25, no special chars)
        invalidLastNames = [
            "VaaaaaaaaaaaaaaaaaaaanGildddddddddder", "", "V@nG1ld3r", 5]
        ...
        invalidAddresss = [[("1234", "Smith St"), "Ashland", "PA", "VA"], ["1234 Smith St", "Ashland", "VA"],
                           [("1234", ""), "Ashland", "VA"], [
                             ("1234", "Smith Sttttttttttttttt"), "Ashland", "VA"], [("1234", 30), "Ashland", "VA"],
                           [(1234, "Smith St"), "Ashland", "VA"], [("", "Smith St"), "Ashland", "VA"], [
                             ("12333333333334", "Smith St"), "Ashland", "VA"],
                           [("1234", "Smith St%"), "Ashland", "VA"], [
                             ("%", "Smith St"), "Ashland", "VA"],
                           [("1234", "Smith St"), 30, "VA"][("1234", "Smith St"), "", "VA"], [
                             ("1234", "Smith St"), "Ashlandddddddddddddddddddddddd", "VA"],
                           [("1234", "Smith St"), "Ashland%", "VA"],
                           [("1234", "Smith St"), "Ashland", "PA"], [
                             ("1234", "Smith St"), "Ashland", ""], [("1234", "Smith St"), "Ashland", 30]
                           ]
        ...
        for i in invalidFirstNames:
            ...
            with self.assertRaises(AssertionError):
                self.client1 = Client(i, "VanGilder", TestClient.CLIENT_ONE_PHONENUM,
        ...
        for i in invalidLastNames:
            # test to ensure asserts occur
            with self.assertRaises(AssertionError):
                 self.client1 = Client("Dillon", i, TestClient.CLIENT_ONE_PHONENUM,
                                       TestClient.CLIENT_ONE_ADDRESS, "Checking", TestClient.CLIENT_ONE_PASSWORD)
        ...
        for i in invalidPhones:
            # test to ensure asserts occur
            with self.assertRaises(AssertionError):
                 self.client1 = Client(TestClient.CLIENT_ONE_FIRSTNAME, TestClient.CLIENT_ONE_LASTNAME,
                                       i, TestClient.CLIENT_ONE_ADDRESS, "Checking", TestClient.CLIENT_ONE_PASSWORD)
        ...
        for i in invalidAddresss:
            ...
            with self.assertRaises(AssertionError):
                self.client1 = Client(TestClient.CLIENT_ONE_FIRSTNAME, TestClient.CLIENT_ONE_LASTNAME,
                                      TestClient.CLIENT_ONE_PHONENUM, i, "Checking", TestClient.CLIENT_ONE_PASSWORD)
        ...
        for i in invalidAccountTypes:
            ...
            with self.assertRaises(AssertionError):
                self.client1 = Client(TestClient.CLIENT_ONE_FIRSTNAME, TestClient.CLIENT_ONE_LASTNAME,
                                      TestClient.CLIENT_ONE_PHONENUM, TestClient.CLIENT_ONE_ADDRESS, i, TestClient.CLIENT_ONE_PASSWORD)
        ...
        for i in invalidPasswords:
            ...
            with self.assertRaises(AssertionError):
                self.client1 = Client(TestClient.CLIENT_ONE_FIRSTNAME, TestClient.CLIENT_ONE_LASTNAME,
                                      TestClient.CLIENT_ONE_PHONENUM, TestClient.CLIENT_ONE_ADDRESS, "Checking", i)

        # setup client
        self.client1 = Client(TestClient.CLIENT_ONE_FIRSTNAME, TestClient.CLIENT_ONE_LASTNAME,
                              TestClient.CLIENT_ONE_PHONENUM, TestClient.CLIENT_ONE_ADDRESS, "Checking", TestClient.CLIENT_ONE_PASSWORD)

    # The test_constructor method tests the constructor
    def testConstructor(self):

        # test regular constructor function and acessor methods
        self.assertEqual(self.client1.getFirstName(),
                         TestClient.CLIENT_ONE_FIRSTNAME)
        self.assertEqual(self.client1.getLastName(),
                         TestClient.CLIENT_ONE_LASTNAME)
        self.assertEqual(self.client1.getPhone(),
                         TestClient.CLIENT_ONE_PHONENUM)
        self.assertEqual(str(self.client1.getAddress()),
                         "1234 Smith St, Ashland, VA")
    ...
    def testPasswordHash(self):
        ...
        self.client2 = Client(TestClient.CLIENT_ONE_FIRSTNAME, TestClient.CLIENT_ONE_LASTNAME,
                              TestClient.CLIENT_ONE_PHONENUM, TestClient.CLIENT_ONE_ADDRESS, "Checking", TestClient.CLIENT_ONE_PASSWORD)
        ...
        self.assertNotEqual(bcrypt.checkpw(
            stored, self.CLIENT_ONE_PASSWORD), True)
        ...
        self.assertNotEqual(self.client2._getPassword()
                            == self.CLIENT_ONE_PASSWORD)

    # test changing client data
    def testChangeInfo(self):

    	# new client
        self.client3 = Client(TestClient.CLIENT_ONE_FIRSTNAME, TestClient.CLIENT_ONE_LASTNAME,
                              TestClient.CLIENT_ONE_PHONENUM, TestClient.CLIENT_ONE_ADDRESS, "Checking", TestClient.CLIENT_ONE_PASSWORD)
        ...
        self.client3.getAddress().updateAddress(
    	    [("751", "Elephant Blvd"), "New Mexico", "VA"])

    	# new client for comparison
        self.client4 = Client("Dawg", "Cat", "8045551234", [(
    	    "751", "Elephant Blvd"), "New Mexico", "VA"], "Checking", TestClient.CLIENT_ONE_PASSWORD)

    	# compare to original
        self.assertNotEqual(self.client3.getFullName(), self.client1.getFullName())
        self.assertNotEqual(self.client3.getPhone(), self.client1.getPhone())
        self.assertNotEqual(self.client3.getAddress(), self.client1.getAddress())

        # test eq methods
        self.assertNotEqual(self.client3.getFullName() == self.client1)
        self.assertNotEqual(self.client3.getPhone() == self.client1)
        self.assertNotEqual(self.client3.getAddress() == self.client1)

        # test str/repr methods
        self.assertEqual(str(self.client3.getFullName()), "Dawg Cat")
        self.assertEqual(str(self.client3.getPhone()), "8045551234")
        self.assertEqual(str(self.client3.getAddress()),
                    "751 Elephant Blvd New Mexico, VA")

        self.assertEqual(self.client3.getFullName().__repr__(), "Dawg Cat")
        self.assertEqual(self.client3.getPhone().__repr__(), "8045551234")
        self.assertEqual(self.client3.getAddress().__repr__(), "751 Elephant Blvd New Mexico, VA")
    ...
    def testOpenAndCloseBankAccount(self):
        ...
        closeAccountBooleanValue = self.client1.closeAccount(
            accountList[0].getAccountNumber())
    ...
    def testToStr(self):
        ...
        self.assertEqual(str(self.client1) == "Client\nClient = {}\nClient Number = {}\nAddress: {}\nPhone = {}\n# of accounts: {}\n".format(fullName, self.client1.getClientNumber(), TestClient.CLIENT_ONE_PHONENUM, TestClient.CLIENT_ONE_ADDRESS, len(self.client1._getAccounts())))
